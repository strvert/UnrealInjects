...
bool UDIContainer::InjectDependenciesToMethods(UObject* Object)
{
	if (!Object)
	{
		UE_LOG(LogDependencyInjection, Error, TEXT("InjectDependenciesToMethods: Object is nullptr"));
		return false;
	}
	...
	for (const FName& FunctionName : FunctionNames)
	{
		...
		{
			UE_LOG(LogDependencyInjection, Log, TEXT("Injecting %s"), *Function->GetName());
			if (!InjectMethodArguments(Object, Function))
			{
				UE_LOG(LogDependencyInjection, Error, TEXT("Failed to inject method arguments (%s)"), *Function->GetName());
				return false;
			}
		}
	}
...
bool UDIContainer::InjectMethodArguments(UObject* Object, UFunction* Method)
{
	...
	for (TFieldIterator<FProperty> PropIt(Method); PropIt && (PropIt->PropertyFlags & CPF_Parm); ++PropIt)
	{
		...
		{
			UE_LOG(LogDependencyInjection, Error, TEXT("Failed to resolve and set field (%s)"), *PropIt->GetName());
			return false;
		}
	}
...
bool UDIContainer::InjectDependenciesToProperties(UObject* Object)
{
	...
	for (TFieldIterator<FProperty> PropIt(ClassType); PropIt; ++PropIt)
	{
		if (const FProperty* Property = *PropIt; Property->HasMetaData("Inject"))
		{
			...
			{
				UE_LOG(LogDependencyInjection, Error, TEXT("Failed to resolve and set field (%s)"), *Property->GetName());
				return false;
			}
		}
	}
...
bool UDIContainer::ResolveAndSetField(void* Container, const FProperty* Property, void* Resolved)
{
	...
	UE_LOG(LogDependencyInjection, Error, TEXT("Property (%s) is not supported"), *Property->GetName());
...
bool UDIContainer::ResolveAndSetArrayField(const void* Container, const FArrayProperty* Property)
{
	...
	if (!InnerClass)
	{
		UE_LOG(LogDependencyInjection, Error, TEXT("Array Property (%s) has no Inner Class"), *Property->GetName());
		return false;
	}
	...
	{
		UE_LOG(LogDependencyInjection, Error, TEXT("Failed to resolve Array Field (%s)"), *Property->GetName());
		return false;
	}
...
bool UDIContainer::ResolveAndSetInterfaceField(void* Container, const FInterfaceProperty* Property, void* Resolved)
{
	...
	if (!ResolvedObject)
	{
		UE_LOG(LogDependencyInjection, Error, TEXT("Failed to resolve Interface (%s)"), *Property->GetName());
		return false;
	}
...
bool UDIContainer::ResolveAndSetClassField(void* Container, const FObjectProperty* Property, void* Resolved)
{
	...
	if (!ResolvedObject)
	{
		UE_LOG(LogDependencyInjection, Error, TEXT("Failed to resolve Class (%s)"), *Property->GetName());
		return false;
	}
