...
void UDITaggedContainerRegistry::Register(const FGameplayTag Tag, UDIContainer* Container)
{
	...
	{
		UE_LOG(LogDependencyInjection, Warning, TEXT("Tagged container already exists for tag %s"), *Tag.ToString());
		return;
	}
	if (!Tag.IsValid())
	{
		UE_LOG(LogDependencyInjection, Warning, TEXT("Invalid tag for container"));
		return;
	}
